...
test("package metadata declares release version 0.1.7", async () => {
  const pkg = JSON.parse(await readFile("package.json", "utf8")) as PackageJson;
...
test("README documents pinned server and TUI plugin install entries", async () => {
  ...
  expect(readme).toContain("opencode-goal-mode@git+https://github.com/YanzuoLu/opencode-goal-mode.git#v0.1.7");
